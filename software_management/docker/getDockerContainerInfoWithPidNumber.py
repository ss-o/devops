...
class dockerContainerUtils(object):
    ...
    def isLegalPid(self):
        if str(self.pid).isdigit() and (int(self.pid) >= 0 or int(self.pid) <= 32768):
            ...
            raise RuntimeError(
                'Pid input should be an integer, and should be 0~32768. Aborted!')
        ...
        try:
            with open(os.path.join(pidDir, "cmdline"), 'r') as f:
                ...
                if cmdline is None or cmdline == '':
                    raise RuntimeError(
                        'cmdline is None or \'\', this always means pid is NOT exist! Aborted! ')
        except IOError:
            print 'pid is not exist! Aborted!'
            sys.exit(1)
        return str(cmdline)
    ...
    def getPPid(self):
        ...
        if len(status) == 0:
            raise KeyError(
                "getStatus(Type: OrderedDict) returns None, this always means pid is NOT exist! Aborted!")
    ...
    def getFDSocketInodeNumber(pid):
        ...
        try:
            ...
            raise RuntimeError(
                "Can NOT open directory \'/proc/%d/fd\'. Aborted!" % pid)
    ...
    def getContainerId(self):
        if self.isDockerRelatedProcess():
            ...
            raise RuntimeError(
                "pid(%s) is not a docker related process's, aborted!" % self.pid)
        string = str(self.getCmdlineChain())
        import re
        ...
        if len(match) != 0 and self.isDockerContainerId(match):
            ...
            raise RuntimeError(
                "not supported docker version or a bug otherwise, aborted! ")
